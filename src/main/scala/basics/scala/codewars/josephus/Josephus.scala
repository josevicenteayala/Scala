...
object Josephus extends App {
  ...
  def josephusSurvivor(n: Int, k: Int): Int = {
    ...
    def josephusRecursive(n: Int, k: Int):Int = {
      n match {
        case 1 => 1
        case _ =>
          println(s"n $n, k $k")
          val response = (josephusRecursive(n - 1, k) + k - 1) % n + 1
          println(s" response $response:: josephusRecursive($n -1, $k) + $k -1) % $n +1")
          response
      }
    }
  ...
  def josephusSurvivor2(n: Int, k: Int): Int = {

    def josephusRecursive(n: Int, k: Int):Int = {
      n match {
        case 1 => 1
        case _ => (josephusRecursive(n - 1, k) + k - 1) % n + 1
      }
    }
    josephusRecursive(n, k)
  }
